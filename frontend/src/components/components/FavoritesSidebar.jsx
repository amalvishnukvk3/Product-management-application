...
const SidebarItem = ({ product, variant, onRemove }) => {
    const navigate = useNavigate();


    return (
        <div
            className="flex p-3 border-b border-gray-100 hover:bg-gray-50 transition cursor-pointer"
        ...
        >
            ...
            {/* Delete button */}
            <button
                className="text-gray-400 hover:text-red-500 ml-2 mt-1 cursor-pointer z-10"
                title="Remove item"
                onClick={(e) => {
...
const FavoritesSidebar = ({ isOpen, onClose }) => {
    ...
    const handleRemove = async (productId) => {
        try {
            const token = localStorage.getItem("token");
            const res = await fetch(`http://localhost:5000/wishlist/${productId}`, {
                method: "DELETE",
                headers: { Authorization: `Bearer ${token}` },
            });
